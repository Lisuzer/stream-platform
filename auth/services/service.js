const series = {
    ...
    getAll: () => {
        return this.series.body;
    },
    getById: (id) => {
        return this.series.body.filter((item) => item.id == id);
    },
    save: (data) => {
        this.series.body.push(data);
        return this.series.body.filter((item) => item.name == data.name);
    },
    saveEpisode: (data) => {
        this.series.body.forEach((item) => {
            if (item.id == data.id) {
                item.episodes.push(data.episode);
            }
        });
        return this.series.body.filter((item) => item.id == data.id);
    },
    update: (id, data) => {
        this.series.body.forEach((item) => {
            if (item.id == id) {
    ...
    delete: (id) => {
        this.series.body.forEach((item, index) => {
            if (item.id == id) {
                this.series.body.splice(index, 1);
            }
        });
        return this.series.body;
    },
